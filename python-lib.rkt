...
(define real-range
  (CFunc empty (hash empty) (some '-args) (none)
         (CReturn (CLet '-length- 'local (CApp (CId 'prim-len) (list (CId '-args)) (hash empty) (none) (none))
               (CIf (Compare '== (CId '-length-) (CNum 1))
                    (CRange (CNum 0) (CIndex (CId '-args) (CNum 0)) (CNum 1))
                    (CIf (Compare '== (CId '-length-) (CNum 2))
                         (CRange (CIndex (CId '-args) (CNum 1)) (CIndex (CId '-args) (CNum 0)) (CNum 1))
                         (CIf (Compare '== (CId '-length-) (CNum 3))
                              (CRange (CIndex (CId '-args) (CNum 1)) (CIndex (CId '-args) (CNum 0)) (CIndex (CId '-args) (CNum 2)))
                              (Cmake-exn "TypeError" "arity mismatch"))))))))
(define lib-functions
  (list (bind 'print (make-prim 'print))
        (bind 'isinstance isinstance)
        (bind 'range (make-prim 'range))
        (bind 'real_range real-range)
